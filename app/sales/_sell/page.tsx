...
export default function SellersPage() {
  ...
  const filteredLeads = useMemo(() => {
    let result = salesLeads.filter(l => l.type === 'seller');
    if (filters.location) {
  ...
  return (
    <div className="flex flex-col min-h-full">
      ...
      />
      <AddLeadModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} department="sales" forcedType="seller" />
        ...
        {filteredLeads.map((lead) => (
            <Link key={lead.id} href={`/leads/${lead.id}`} className="block bg-white p-3 rounded-xl shadow-sm border border-gray-100 relative overflow-hidden active:scale-[0.98] transition-transform">
              ...
              <div className="flex justify-between items-center pl-2">
                ...
                {/* @ts-ignore */}
                <p className="font-bold text-sm text-blue-600">{lead.demand}</p>
              </div>
            </Link>
        ))}
      </div>
