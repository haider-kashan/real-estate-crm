...
export default function UserMenu({ email, name }: { email: string; name: string }) {
  ...
  return (
    <div className="relative" ref={menuRef}>
      {/* 1. THE TRIGGER BUTTON (Avatar) */}
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="flex items-center justify-center w-10 h-10 rounded-full bg-black text-white font-bold text-sm hover:bg-gray-800 transition-all border-2 border-white shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-black"
      >
        {initials}
      </button>
