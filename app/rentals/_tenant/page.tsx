...
export default function TenantPage() {
  ...
  const filteredLeads = useMemo(() => {
    // 1. Start with ONLY Tenants
    let result = rentalLeads.filter(l => l.type === 'tenant');
  ...
  return (
    <div className="flex flex-col min-h-full">
      ...
      <HeaderBar 
        title="Tenants List" 
        subtitle={`${filteredLeads.length} Active Requests`}
        onFilterChange={setFilters} 
        onSortChange={setSortBy} 
        onAddClick={() => setIsModalOpen(true)}
      />
      
      {/* Modal forced to 'tenant' */}
      <AddLeadModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} department="rentals" forcedType="tenant" />
          ...
          filteredLeads.map((lead) => (
            <Link key={lead.id} href={`/leads/${lead.id}`} className="block bg-white p-3 rounded-xl shadow-sm border border-gray-100 relative overflow-hidden active:scale-[0.98] transition-transform">
              ...
              <div className="flex justify-between items-center pl-2">
                ...
                {/* @ts-ignore */}
                <p className="font-bold text-sm text-indigo-600">{lead.budget}</p>
              </div>
            </Link>
          ))
        )}
